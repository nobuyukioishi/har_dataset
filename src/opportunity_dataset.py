...
class OpportunityDataset:
    ...
    @staticmethod
    def download_opportunity(save_dir: Union[Path, str] = "./data/", archive_url: str = ARCHIVE_URL):
        """
        ...
        if type(save_dir) != PosixPath:
            save_dir = Path(save_dir)

        if not save_dir.is_dir():
            raise FileNotFoundError(
                errno.ENOENT, os.strerror(errno.ENOENT),
                f"{save_dir} not found. Please specify a directory that already exists."
            )

        res = requests.get(archive_url, stream=True)
        chunk_size = 1024
        with open(save_dir / "OpportunityUCIDataset.zip", "wb") as f:
            ...
            print(f"OpportunityUCIDataset.zip has been saved in {save_dir}")
